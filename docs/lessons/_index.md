...
| 011 | 2026-09-22 | PR-Agent's review job goes red on a fully `[ignore]`d diff and blames the wrong cause | [lesson-011-pr-agent-review-job-goes-red-on-a-fully-ignored-diff.md](lesson-011-pr-agent-review-job-goes-red-on-a-fully-ignored-diff.md) | `ci`, `pr-agent`, `review-gate`, `misdiagnosis` |
| 012 | 2026-09-22 | The GraphQL quota is per *user*, so one session's board queries can block a CI release | [lesson-012-graphql-quota-is-per-user-so-one-session-can-block-ci-releases.md](lesson-012-graphql-quota-is-per-user-so-one-session-can-block-ci-releases.md) | `github`, `graphql`, `rate-limit`, `ci`, `gotcha` |
